...
import { SONGS_FETCH_INIT } from './songsSource'
...
export async function checkForUpdate(url, storedEtag) {
  try {
    // `no-cache`: ETag, прочитанный из кэша браузера, всегда равен
    // сохранённому — проверка была бы слепа к обновлению (см. lib/songsSource.js).
    const response = await fetch(url, { method: 'HEAD', ...SONGS_FETCH_INIT })
    if (!response.ok) {
