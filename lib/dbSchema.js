...
export const createSchema = (db) => {
    ...
    // Настройки конкретной песни: сейчас это только сдвиг тональности, но
    // хранилище заводится общим — то же значение будет двигать партитуру, и
    // переезжать на общее поле потом дороже, чем сделать так сразу.
    // Текст песни при этом остаётся в исходной тональности: сдвиг применяется
    // при отрисовке, иначе исходник портился бы необратимо.
    if (!db.objectStoreNames.contains('songSettings')) {
        db.createObjectStore('songSettings', { keyPath: 'songNumber' })
    }
