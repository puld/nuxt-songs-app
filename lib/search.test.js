...
describe('performExactSearch', () => {
    ...
    it('должен находить по началу фразы (части символов из нескольких слов)', () => {
        ...
        const gluingIndex = buildExactIndex(gluingSongs)
        const results = performExactSearch(gluingIndex, 'как пр', 10)
        expect(results.length).toBe(1)
        expect(results[0].n).toBe('1')
    })
    ...
    it('должен сортировать результаты по релевантности (score), затем по алфавиту', () => {
        const results = performExactSearch(index, 'дождь', 10)
    ...
    it('должен ставить title-совпадения выше body-совпадений', () => {
        ...
        const priorityIndex = buildExactIndex(prioritySongs)
        const results = performExactSearch(priorityIndex, 'как пр', 10)
        expect(results[0].n).toBe('2')
    })
