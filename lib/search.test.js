...
describe('cleanText', () => {
    it('должен удалять спецсимволы', () => {
        ...
        expect(cleanText('Скобки (тест) и ещё')).toBe('Скобки тест и ещё')
    })

    it('убирает ремарку вместе с её текстом', () => {
        // Круглые скобки лишь снимаются — внутри них слова песни (подголоски).
        // Квадратные уносят содержимое: это ремарка исполнителю.
        expect(cleanText('Строка (подголосок) [Припев можно петь через два куплета]'))
            .toBe('Строка подголосок')
    })
...
describe('stripRemarks', () => {
    it('убирает ремарку целиком', () => {
        expect(stripRemarks('[Припев можно петь через два куплета]').trim()).toBe('')
    })

    it('оставляет текст вокруг ремарки', () => {
        const text = 'Будем гибнущих искать.\n[В скобках – подголоски]'
        expect(stripRemarks(text)).toContain('Будем гибнущих искать.')
        expect(stripRemarks(text)).not.toContain('подголоски')
    })

    it('не склеивает слова по краям ремарки', () => {
        expect(stripRemarks('до[ремарка]после')).toBe('до после')
    })

    it('убирает ремарку, разбитую на строки', () => {
        expect(stripRemarks('[первая строка\nвторая строка]').trim()).toBe('')
    })

    it('текст без ремарок не меняется', () => {
        expect(stripRemarks('Обычная строка песни')).toBe('Обычная строка песни')
    })

    it('незакрытая скобка оставляет текст на месте', () => {
        // Половина ремарки — скорее опечатка в данных, чем ремарка; терять
        // строку песни из-за одного символа нельзя.
        expect(stripRemarks('Строка песни [без закрытия')).toBe('Строка песни [без закрытия')
    })

    it('нестроковое значение даёт пустую строку', () => {
        expect(stripRemarks(null)).toBe('')
        expect(stripRemarks(undefined)).toBe('')
    })
})
...
describe('prepareSongForIndexing', () => {
        ...
        variants: [
            {
                label: '',
                body: [
                    { type: 'verse', content: 'Куплет с (подголоском) и текстом...' },
    ...
    it('должен подготавливать песню для индексации', () => {
        ...
        expect(result).toEqual({
            n: 1,
            title: 'Песня тестовая',
            content: 'Куплет с подголоском и текстом Припев лалала'
        })
    })
    ...
    it('не индексирует ремарку исполнителю', () => {
        // Форма как в данных (#902): ремарка — последняя строка строфы.
        const songWithRemark = {
            number: 6,
            title: 'Велик',
            variants: [
                {
                    ...
                    body: [
                        {
                            type: 'verse',
                            content: '«Велик!» – говорили цветы.\n[Припев можно петь через два куплета]'
                        }
                    ]
                }
            ]
        ...
        const result = prepareSongForIndexing(songWithRemark)
        expect(result.content).toContain('Велик')
        expect(result.content).not.toContain('куплета')
    })
})
...
describe('Ремарки исполнителю не попадают в поиск', () => {
    // Слова из ремарок («припев», «куплет», «подголоски») в самих песнях
    // встречаются редко, поэтому до чистки такой запрос выдавал ровно те
    // песни, где ремарка есть, — выдача выглядела осмысленной и была ложной.
    const songs = [
        {
            number: 902,
            title: 'Велик Господь',
        ...
        },
        {
            number: 903,
            title: 'Другая песня',
            variants: [{ label: '', body: [{ type: 'verse', content: 'Совсем другие слова' }] }]
        }
    ]

    it('Lunr-поиск по слову из ремарки ничего не находит', () => {
        const index = buildSearchIndex(songs)

        expect(performSearch(index, 'куплета', 10)).toEqual([])
    })

    it('точный поиск по фразе из ремарки ничего не находит', () => {
        const index = buildExactIndex(songs)

        expect(performExactSearch(index, 'можно петь', 10)).toEqual([])
    })

    it('текст самой песни при этом находится', () => {
        const index = buildExactIndex(songs)
        const results = performExactSearch(index, 'говорили цветы', 10)

        expect(results.map(r => r.n)).toEqual(['902'])
    })
})
