...
/**
 * Схлопывает подряд идущие аккорды с одинаковым корнем: у второго и
 * последующих снимается разметка целиком (текст под ними остаётся), у
 * первого — нет. `C → C7 → C` превращается в один `C`: суффикс и бас на
 * решение не влияют, важен только корень (буква + знак).
 *
 * В отличие от `simplifyChord`, это не подмена аккорда, а решение вообще его
 * не показывать — поэтому отдельный тоггл, зависимый от общего упрощения:
 * без него на экране осталась бы всё та же партитурная плотность, просто с
 * упрощёнными обозначениями.
 *
 * Аккорды из разных проходов повтора (`{1:C}`… `{2:C}`) не схлопываются
 * между собой, даже при совпавшем корне — это разная гармония по смыслу,
 * а не одна и та же ступень, зафиксированная дважды.
 */
export function collapseRepeatedRootsText(text) {
  if (!text) return text || ''
  let previous = null
  return String(text).replace(MARKUP_RE, (whole, pass, inline, chord) => {
    const parsed = parseChord(chord)
    ...
    const key = pass || ''
    if (previous && previous.root === parsed.root && previous.pass === key) {
      return ''
    }
    previous = { root: parsed.root, pass: key }
    return whole
  })
}
