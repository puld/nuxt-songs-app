...
import { isPersistSupported, requestPersistentStorage, readPersisted, getStorageEstimate, formatBytes } from './storagePersist'
...
describe('readPersisted', () => {
    it('возвращает выданный флаг, не запрашивая его', async () => {
        const persist = vi.fn()
        const storage = { persist, persisted: async () => true }

        expect(await readPersisted(storage)).toEqual({ supported: true, persisted: true })
        expect(persist).not.toHaveBeenCalled()
    })

    it('невыданный флаг тоже читает без запроса', async () => {
        const persist = vi.fn()
        const storage = { persist, persisted: async () => false }

        expect(await readPersisted(storage)).toEqual({ supported: true, persisted: false })
        expect(persist).not.toHaveBeenCalled()
    })

    it('без API сообщает об отсутствии поддержки', async () => {
        expect(await readPersisted({})).toEqual({ supported: false, persisted: null })
    })

    it('исключение — «неизвестно», а не «нет»', async () => {
        const storage = { persist: () => {}, persisted: async () => { throw new Error('приватный режим') } }

        expect(await readPersisted(storage)).toEqual({ supported: true, persisted: null })
    })
})
