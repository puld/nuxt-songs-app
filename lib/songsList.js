...
/** Режимы группировки — порядок совпадает с порядком кнопок на странице. */
export const SONGS_LIST_MODES = ['number', 'alphabet', 'sections']

/** Режим по умолчанию: номер — единственный способ найти песню, который есть в бумажном сборнике. */
export const DEFAULT_SONGS_LIST_MODE = 'number'

/**
 * Приводит режим к допустимому значению.
 *
 * Режим приходит из localStorage, где может оказаться что угодно — значение от
 * будущей версии, ручная правка, мусор. Без нормализации страница сгруппировала
 * бы песни по номеру (ветка по умолчанию в `groupSongs`), но ни одна кнопка не
 * подсветилась бы — выглядело бы как поломка.
 *
 * @param {string} mode
 * @returns {'number'|'alphabet'|'sections'}
 */
export const normalizeSongsListMode = (mode) => (
    SONGS_LIST_MODES.includes(mode) ? mode : DEFAULT_SONGS_LIST_MODE
)
 ...
 * Ключ группы раздела.
 *
 * Ключ, а не название: в сборнике есть разделы с одинаковыми названиями.
 * Вынесен в функцию, потому что по этому же ключу страница «Все песни»
 * раскрывает нужную группу при переходе со страницы песни.
 */
export const sectionGroupKey = (id) => `section-${id}`

/**
 * Якорь раздела для ссылки на «Все песни».
 *
 * Переход оформлен якорем, а не query-параметром, потому что при навигации
 * внутри приложения роутер прокручивает страницу уже после `onMounted`: без
 * hash он вернул бы её наверх, и раскрытая группа осталась бы за экраном.
 */
export const sectionAnchor = (id) => `#${sectionGroupKey(id)}`

/**
 * Ключ группы раздела из якоря URL или null, если якорь не про раздел.
 *
 * Хеш приходит из адресной строки, поэтому проверяем формат: чужой якорь не
 * должен раскрывать случайную группу.
 */
export const sectionKeyFromHash = (hash) => {
    const match = /^#(section-\d+)$/.exec(String(hash || ''))
...
export const groupBySections = (songs, sections) => {
    ...
    const groups = (sections || []).map((section) => {
        ...
        return {
            key: sectionGroupKey(section.id),
