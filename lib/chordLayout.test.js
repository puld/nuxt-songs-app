...
describe('planChordShifts: границы строки', () => {
  ...
  it('сосед не выталкивает группу за правый край', () => {
    // Две пары впритык: первая раздвигается и подпирает вторую, а той уже некуда вправо
    const labels = [label(0, 0, 40), label(0, 10, 40), label(0, 100, 40), label(0, 110, 40)]
    const shifts = planChordShifts(labels, { minLeft: 0, maxRight: 170 })
    const right = Math.max(...labels.map((l, i) => l.left + shifts[i] + l.width))
    expect(right).toBeLessThanOrEqual(170)
  })

  it('места нет вовсе: группа ужимается в строку, а не уезжает за правый край', () => {
    const labels = [label(0, 0, 40), label(0, 5, 40), label(0, 10, 40)]
    const shifts = planChordShifts(labels, { minLeft: 0, maxRight: 100 })
    expect(labels[0].left + shifts[0]).toBe(0)
    const right = Math.max(...labels.map((l, i) => l.left + shifts[i] + l.width))
    expect(right).toBeCloseTo(100)
  })

  it('ужатая группа сохраняет порядок надписей', () => {
    const labels = [label(0, 0, 40), label(0, 5, 40), label(0, 10, 40)]
    const shifts = planChordShifts(labels, { minLeft: 0, maxRight: 100 })
    const lefts = labels.map((l, i) => l.left + shifts[i])
