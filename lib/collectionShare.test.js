...
describe('encodeShare / decodeShare', () => {
    ...
    it('ошибка сборки доходит до вызывающего', async () => {
        await expect(encodeShare({ ...collection(), songs: [] }))
            .resolves.toEqual({ ok: false, data: '', error: 'В подборке нет песен' })
    })
    ...
    it('сжатая ссылка честно говорит про версию приложения', async () => {
        // Формат 2 появится в 4.5; сейчас такая ссылка может прийти только
        // от более новой версии, и «испорчена» было бы неправдой.
        const gzipLike = bytesToBase64Url(new Uint8Array([50, 10, 31, 139, 8]))

        const result = await decodeShare(gzipLike)
