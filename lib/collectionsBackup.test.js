...
describe('порядок подборок в копии', () => {
    const at = (day) => `2026-01-0${day}T00:00:00.000Z`

    it('order переносится в копию', () => {
        const backup = buildBackup([
            { id: 1, name: 'Пасха', order: 2 },
            { id: 2, name: 'Рождество', order: 0 }
        ], [], at(1))

        expect(backup.collections.map((c) => c.order)).toEqual([2, 0])
    })

    it('нулевой order не теряется', () => {
        // Первая подборка в списке — самый обычный случай, а `if (order)` его бы съел.
        const backup = buildBackup([{ id: 1, name: 'Пасха', order: 0 }], [], at(1))

        expect(backup.collections[0].order).toBe(0)
    })

    it('мусорный order в копию не попадает', () => {
        // Правка файла руками или формат от будущей версии.
        const cases = [{ order: 'первая' }, { order: -1 }, { order: 1.5 }, { order: null }, { order: '' }, { order: NaN }]

        for (const extra of cases) {
            const backup = buildBackup([{ id: 1, name: 'Пасха', ...extra }], [], at(1))

            expect(backup.collections[0].order).toBeUndefined()
        }
    })

    it('копия без order разбирается как раньше', () => {
        // Совместимость с копиями, снятыми до появления сортировки.
        const parsed = parseBackup(JSON.stringify({
            v: 1,
            savedAt: at(1),
            collections: [{ id: 1, name: 'Пасха' }],
            links: []
        }))

        expect(parsed.ok).toBe(true)
        expect(parsed.backup.collections[0].order).toBeUndefined()
    })

    it('план импорта идёт в порядке показа, а не в порядке записей', () => {
        // Записи приходят в порядке `id` (так их отдаёт IndexedDB), а создавать
        // надо в порядке сайдбара: нумерацию выдаёт `createCollection`.
        const backup = buildBackup([
            { id: 1, name: 'Пасха', order: 2 },
            { id: 2, name: 'Рождество', order: 0 },
            { id: 3, name: 'Псалмы', order: 1 }
        ], [], at(1))

        const plan = planImport(backup, [])

        expect(plan.map((item) => item.name)).toEqual(['Рождество', 'Псалмы', 'Пасха'])
    })

    it('«Избранное» в плане первым, даже если его order больше', () => {
        const backup = buildBackup([
            { id: 1, name: 'Пасха', order: 0 },
            { id: 2, name: 'Избранное', isFavorite: 1, order: 5 }
        ], [], at(1))

        const plan = planImport(backup, [])

        expect(plan.map((item) => item.name)).toEqual(['Избранное', 'Пасха'])
    })

    it('без order план строится по датам создания', () => {
        const backup = buildBackup([
            { id: 1, name: 'Псалмы', createdAt: at(3) },
            { id: 2, name: 'Пасха', createdAt: at(1) }
        ], [], at(4))

        const plan = planImport(backup, [])

        expect(plan.map((item) => item.name)).toEqual(['Пасха', 'Псалмы'])
    })

    it('связи не теряются при переупорядочивании плана', () => {
        // Связи привязаны к `id` подборки, а план после сортировки идёт в другом
        // порядке — легко потерять, если брать их по индексу.
        const backup = buildBackup(
            [{ id: 1, name: 'Пасха', order: 1 }, { id: 2, name: 'Рождество', order: 0 }],
        ...
        expect(plan[0]).toMatchObject({ name: 'Рождество', links: [] })
        expect(plan[1].links).toEqual([{ songNumber: 115, variantIndex: 0 }])
    })
})
