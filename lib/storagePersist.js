...
/**
 * Читает текущее состояние флага, ничего не запрашивая — для диагностики.
 *
 * Отдельно от `requestPersistentStorage`, потому что тот при отсутствии флага
 * шлёт запрос и может показать промпт; блок диагностики должен только смотреть.
 *
 * @returns {Promise<{supported: boolean, persisted: boolean|null}>}
 */
export const readPersisted = async (storage) => {
    if (!isPersistSupported(storage)) return { supported: false, persisted: null }
