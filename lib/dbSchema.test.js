...
describe('dbSchema', () => {
    ...
    it('createSchema создаёт четыре хранилища', async () => {
        ...
        expect([...db.objectStoreNames].sort()).toEqual(['collections', 'sections', 'songCollections', 'songs'])
        db.close()
    })
    ...
    it('идемпотентна: повторный вызов на существующих хранилищах не падает', async () => {
        ...
        expect([...second.objectStoreNames].sort()).toEqual(['collections', 'sections', 'songCollections', 'songs'])
        second.close()
    })
})
