...
describe('dbSchema', () => {
    ...
    it('createSchema создаёт четыре хранилища', async () => {
        const db = await openWithSchema(new IDBFactory())

        expect([...db.objectStoreNames].sort()).toEqual(['collections', 'sections', 'songCollections', 'songSettings', 'songs'])
        db.close()
    })
    ...
    it('идемпотентна: повторный вызов на существующих хранилищах не падает', async () => {
        ...
        expect([...second.objectStoreNames].sort()).toEqual(['collections', 'sections', 'songCollections', 'songSettings', 'songs'])
        second.close()
    })
})
