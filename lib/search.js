 ...
 * как целое слово, в исходном порядке (между словами запроса могут быть другие слова).
 */
export const performExactSearch = (searchIndex, query, limit = 0) => {
    ...
    const matchesWords = (textWords) => {
        let i = 0
        for (const w of textWords) {
            if (i < terms.length && w === terms[i]) i++
    ...
    const results = searchIndex.filter(s => {
        const textWords = s.text.split(/\s+/)
        if (isMultiWord) {
            return matchesWords(textWords)
        }
        return textWords.some(w => w === query)
    })
    ...
    const scored = results.map(s => {
        ...
        let idx = -1
        if (isMultiWord) {
            const fi = textWords.findIndex((w, i) =>
                w === terms[0] && textWords.slice(i, i + terms.length).every((tw, j) => tw === terms[j])
            )
            if (fi >= 0) idx = s.text.indexOf(textWords[fi])
        } else {
            const fi = textWords.findIndex(w => w === query)
            if (fi >= 0) idx = s.text.indexOf(textWords[fi])
        }
