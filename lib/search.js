...
/**
 * Выполняет точный поиск по символам: пробелы (и регистр) выкидываются,
 * ищется как подстрока в склеенном тексте песни.
 * - "дож" найдёт "дождь"
 * - "как пр" найдёт "как прекрасны"
 */
export const performExactSearch = (searchIndex, query, limit = 0) => {
    ...
    const results = searchIndex.filter(s => s.textFlat.includes(normalizedQuery))

    const scored = results.map(s => {
        const idx = s.textFlat.indexOf(normalizedQuery)
