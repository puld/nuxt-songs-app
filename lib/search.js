...
/**
 * Убирает ремарки — пояснения в квадратных скобках («[Припев можно петь через
 * два куплета]»). Это указание исполнителю, а не слова песни: на экране ремарка
 * нужна, но в индексе она даёт ложную выдачу — по запросу «куплет» находились
 * песни, где такого слова нет.
 *
 * Снимать одни скобки недостаточно: тогда в индекс попадает сам текст ремарки.
 * Заменяем на пробел, а не на пустую строку, чтобы «слово[ремарка]слово» не
 * склеилось в один терм.
 *
 * @param {string} text - Исходный текст
 * @returns {string} Текст без ремарок
 */
export const stripRemarks = (text) => {
    if (typeof text !== 'string') return ""
    return text.replace(/\[[^\]]*\]/g, ' ')
}

/**
 * Очищает текст от ремарок и спецсимволов, нормализует пробелы
 * @param {string} text - Исходный текст
 * @returns {string} Очищенный текст
 */
export const cleanText = (text) => {
    if (text === null) return ""
    return stripRemarks(text)
        .replace(/[/\[\]()!?.,;:"'-]/g, ' ')
        .replace(/\s+/g, ' ')
        .trim()
}
