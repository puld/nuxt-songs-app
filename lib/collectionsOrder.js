...
/**
 * Ограничивает смещение перетаскиваемой строки границами списка.
 *
 * Список подборок скроллируется (`overflow-y: auto`), а строка едет на
 * `transform`: уехав за край, она обрезается контейнером — палец продолжает
 * тащить то, чего не видно. Дальше крайних слотов двигать всё равно некуда,
 * `dropIndex` там упирается в ту же границу.
 *
 * @param {number} from — позиция строки в списке
 * @param {number} deltaY — смещение в пикселях (в координатах содержимого)
 * @param {number} rowHeight — высота строки
 * @param {number} count — сколько строк переставляется
 * @returns {number} смещение, при котором строка остаётся в пределах списка
 */
export const clampOffset = (from, deltaY, rowHeight, count) => {
    if (!Number.isFinite(deltaY)) return 0
    if (!Number.isInteger(from) || !Number.isInteger(count) || count <= 0) return deltaY
    if (!Number.isFinite(rowHeight) || rowHeight <= 0) return deltaY

    const clamped = Math.min(Math.max(deltaY, -from * rowHeight), (count - 1 - from) * rowHeight)

    // `-0` у крайней строки: в стиле он безвреден, но в сравнениях и тестах
    // ведёт себя не как ноль, и разбираться с этим потом дороже.
    return clamped === 0 ? 0 : clamped
}

/** Полоса у края списка, в которой он подкручивается сам (px). */
export const AUTO_SCROLL_EDGE = 36

/**
 * Скорость автоскролла, px/с.
 *
 * Именно скорость, а не шаг за кадр: шаг привязал бы прокрутку к частоте
 * обновления экрана — на 120 Гц список ехал бы вдвое быстрее, чем на 60, и
 * подобрать комфортное значение стало бы невозможно.
 *
 * Порядка пяти строк в секунду: список надо успевать читать на ходу, иначе
 * подборку проносит мимо нужного места и жест приходится начинать заново.
 */
export const AUTO_SCROLL_SPEED = 240

/**
 * Кадр длиннее этого считаем провалом (переключение вкладки, лаг отрисовки):
 * умножать скорость на всю паузу нельзя — список прыгнул бы разом.
 */
const MAX_FRAME_MS = 50

/**
 * На сколько прокрутить список за кадр, пока указатель держат у его края.
 *
 * @param {number} pointerY — координата указателя в координатах окна
 * @param {number} top — верхняя граница списка
 * @param {number} bottom — нижняя граница списка
 * @param {number} elapsed — длительность кадра, мс
 * @param {number} edge — ширина полосы у края
 * @param {number} speed — скорость, px/с
 * @returns {number} смещение со знаком; 0 — указатель не у края
 */
export const autoScrollStep = (
    pointerY, top, bottom, elapsed, edge = AUTO_SCROLL_EDGE, speed = AUTO_SCROLL_SPEED
) => {
    if (![pointerY, top, bottom, elapsed, edge, speed].every(Number.isFinite)) return 0
    if (elapsed <= 0 || speed <= 0) return 0

    const distance = speed * Math.min(elapsed, MAX_FRAME_MS) / 1000

    // Верх приоритетнее: в списке ниже двух полос края зоны перекрываются, и
    // без явного порядка выбор направления зависел бы от порядка проверок.
    if (pointerY - top < edge) return -distance
    if (bottom - pointerY < edge) return distance
