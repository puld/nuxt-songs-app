...
describe('якорь раздела', () => {
    it('ссылка на раздел указывает на группу списка', () => {
        expect(sectionAnchor(21)).toBe(`#${sectionGroupKey(21)}`)
    })

    it('ключ группы разбирается обратно из якоря', () => {
        expect(sectionKeyFromHash(sectionAnchor(0))).toBe(sectionGroupKey(0))
        expect(sectionKeyFromHash('#section-46')).toBe('section-46')
    })

    it('чужой якорь раздел не открывает', () => {
        // Иначе ссылка на любой другой элемент страницы раскрывала бы группу.
        expect(sectionKeyFromHash('#top')).toBeNull()
        expect(sectionKeyFromHash('#section-')).toBeNull()
        expect(sectionKeyFromHash('#section-abc')).toBeNull()
        expect(sectionKeyFromHash('section-1')).toBeNull()
    })

    it('пустой якорь — обычный заход на страницу', () => {
        expect(sectionKeyFromHash('')).toBeNull()
        expect(sectionKeyFromHash(undefined)).toBeNull()
        expect(sectionKeyFromHash(null)).toBeNull()
    })
})
