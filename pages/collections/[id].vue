<template>
  <ClientOnly>
    <Teleport to="#navbar-center">
      <span class="nav-title">{{ isFavorites ? '♥ Избранное' : collection.name }}</span>
    </Teleport>
  </ClientOnly>
  ...
  <div>
    ...
    <div v-else-if="isFavorites">
      ...
      <div v-if="songs.length === 0" class="empty">
        ...
        <NuxtLink to="/library">Добавить в избранное</NuxtLink>
      </div>
...
onMounted(async () => {
  try {
    if (isFavorites.value) {
      ...
      const favNumbers = new Set(favoritesStore.favoriteSongs)
      songs.value = allSongs.value.filter(s => favNumbers.has(s.number))
    } else {
