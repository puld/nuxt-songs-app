...
describe('useSongsCache', () => {
    ...
    describe('ошибки', () => {
        it('неудачная загрузка не кэшируется — следующая попытка читает базу', async () => {
            ...
            // База недоступна: getAllSongs упадёт на обращении к $indexedDB.
            // Инстанс composable захватывает $indexedDB в момент вызова,
            // поэтому «сломанный» и «рабочий» инстансы создаём отдельно —
            // как это и происходит при монтировании компонентов.
            mockDBRef.current = null
