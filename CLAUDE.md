...
- **Порядок подборок в копию не входит** (см. «Порядок подборок»), и формат от его появления не менялся: `pickCollection` собирает только `id`, `name`, `isFavorite`, даты — `order` из файла отбрасывается, даже если его туда кто-то дописал. Восстановленные подборки получают нумерацию заново от `createCollection` (`nextOrder`), поэтому чужие номера не сталкиваются с уже расставленными. Обратная сторона: расстановка не переносится — подборки встают в порядке записей в файле. Покрыто в `composables/useCollectionsBackup.test.js`, блок «порядок подборок и копия»
