...
Аккорды размечены в 65 песнях из 1565, и до открытия песни узнать об этом было
...
- Тумблер аккордов — за `settings.devMode`: аккорды расставлены в 65 песнях из 1565 (5.4 в дорожной карте), и обычному пользователю они попадались бы через раз. Тумблер — **единственное** место, где показ аккордов включается; условие показа собирает геттер `chordsVisible` (см. «Хранилище настроек»)
