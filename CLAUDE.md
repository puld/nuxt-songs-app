...
- **Имя правится получателем, и считается всё от поля, а не от ссылки.** Присланное
  «Рождество» осмысленно у отправителя, а у получателя таких ссылок может быть несколько
  от разных людей — различать их иначе нечем. Поэтому и совпадение с существующей
  подборкой, и свободное имя рядом с ней (`Рождество (2)`) берутся из поля: вписав имя
  своей подборки, получатель включает слияние, а вписав своё — сохраняет отдельно.
  Пустое имя сохранять нельзя (подборка без названия неотличима в сайдбаре), кнопки
  заблокированы. Заголовок остаётся присланным именем — он говорит, что прислали, а поле
  говорит, куда это ляжет
- **Правило «та же подборка» одно с резервной копией**: `normalizeCollectionName`
  ...
  подборку или сохранить рядом под свободным именем
