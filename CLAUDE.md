...
### Высота страницы — только dvh, не 100% и не vh
Цепочка `html, body` → `#__nuxt` (`assets/css/main.css`) → `.layout` (`layouts/default.vue`) тянется по `min-height: 100dvh` с фолбэком `100vh`. Проценты и `vh` считаются от large-вьюпорта, который в PWA на Android **не** пересчитывается, когда системная навигация вызывается свайпом снизу: окно уменьшается, высота остаётся прежней — на короткой странице появляется вертикальный скролл, содержимое уезжает вверх. `dvh` пересчитывается вместе с окном.

Регрессию сторожит e2e «высота страницы привязана к динамическому вьюпорту (dvh)» (`responsive.spec.js`) — он проверяет **объявление** в CSSOM, а не поведение: в desktop-Chromium large-вьюпорт всегда равен окну, расхождение large/small там не воспроизводится, и поведенческая проверка была бы зелёной и без фикса.
