...
### `pages/settings/index.vue` — Настройки
...
- Под ним — ссылка на `/settings/chords` (только при включённом показе аккордов): упрощение для гитары, диезы/бемоли, немецкая нотация. Тумблеров под аккорды набралось несколько — на одной странице с темой, шрифтом и резервной копией они бы её заслонили, поэтому вынесены отдельно
- Секция «Экспериментальные функции» показывается только при `settings.devMode`; там же тумблер, которым режим выключается

### `pages/settings/chords.vue` — Настройки аккордов

Отдельная страница ради четырёх тогглов, которые иначе перегружали бы общие
настройки. Как и `/songs`, `/collections/import` — гейт `devMode` скрывает
вход, но не закрывает маршрут (`ssr: false` генерирует статику для всех
страниц), поэтому без режима разработчика показывается заглушка со ссылкой
назад в «Настройки».

**Файл `settings.vue` переименован в `settings/index.vue`.** Каталог
`pages/settings/` с файлом `pages/settings.vue` рядом — конфликт: Nuxt
трактует их как родитель/потомок и без `<NuxtPage/>` в родителе дочерний
маршрут не рендерится вовсе (URL меняется, контент остаётся родительским).
`index.vue` в каталоге — тот же маршрут `/settings`, но без коллизии: обе
страницы становятся независимыми, а не вложенными.

Каждый тоггл **заблокирован, а не скрыт**, пока `settings.showChords`
выключен — та же причина, что раньше была в `settings.vue`: исчезающая
строка дёргала бы страницу при каждом переключении, а о самой возможности
узнать было бы неоткуда. Классы `simplify-toggle`/`force-sharp-toggle`/
`german-notation-toggle` — ради e2e: общий `.toggle-switch.sub-toggle`
...
- **Немецкая нотация** (`germanNotation`) — си → `H`, си-бемоль → `B` вместо
  ...
  1-м ладу — «B» (не A#/Bb — так пишут в русской и немецкой традиции
  сольфеджио), на 2-м — «H» (не B — так по-английски), на 3-м — «Cm». Это не
  вариант диезного/бемольного набора, а замена буквы для одной конкретной
  пары ступеней: `noteName(pitch, sharp, german)` в `lib/transpose.js`
  проверяет `german` **раньше** выбора между `SHARP_NAMES`/`FLAT_NAMES` и
  возвращает `B`/`H` напрямую, не трогая остальные десять ступеней. Работает
  и без сдвига тональности (`transposeText` не выходит рано при
  `german=true`, как и при `forceSharp`), и перебивает `forceSharp` на этой
  паре ступеней, если включены оба — они независимы, но `german` главнее
  ровно для si/si-bemol. `songKey` в панели тональности (`ChordControls.vue`)
  получил третий и четвёртый параметр (`forceSharp`, `german`) той же
  сессией, что и сама немецкая нотация — раньше панель не знала даже про
  `forceSharp`, и диезный тоггл менял надписи на листе, но не подпись
  тональности в шапке
...
### Аккорды упрощаются для гитары по настройке

Аккорды сняты не с гитарных табов, а с партитур сборника (5.4 в дорожной карте) —
там встречаются обращения (`G/B`, `D7/F#`) и sus-задержания, уменьшённые,
полууменьшённые и увеличенные трезвучия (`Bb7(sus4)`, `Adim`, `Am7(b5)`, `G+`),
которые для простого гитарного аккомпанемента избыточны. Оба вида упрощения
собраны **в одном тоггле** `simplifyChords` (геттер `chordsSimplified`,
тумблер «Упростить для гитары» на `/settings/chords`) — до объединения бас
прятался отдельным ключом `hideChordBass`, но для читателя это один и тот же
вопрос «что реально сыграть», а не две разные настройки. Чистые функции —
`stripChordBass`/`stripBassText` (только бас) и `simplifyChordSuffix`/
`simplifyChord`/`simplifyChordsText` (бас **и** суффикс, единая точка входа)
в `lib/transpose.js`.

- **Миграция старого значения.** У части пользователей `hideChordBass: true`
  уже лежал в localStorage до объединения. `migrateHideChordBass()` в
  `stores/settings.js` при первом обращении к стору переносит его в
  `simplifyChords`, если новый ключ ещё не создан — иначе апдейт молча выключил
  бы упрощение у тех, кто уже им пользовался. Перенос односторонний и разовый:
  дальше ключи независимы, а `hideChordBass` в localStorage остаётся
  нетронутым мусором (никто его больше не читает)
- **`simplifyChord` снимает бас первым шагом**, потом упрощает суффикс
  оставшегося аккорда — порядок неважен для результата (это разные части
  строки), но так проще: `parseChord` после `stripChordBass` уже не видит баса
  вовсе
- **Список сложных обозначений закрытый, а не общий разбор джазовой нотации.**
  В сборнике эти суффиксы конечны и известны наперёд (см. подсчёт по корпусу в
  истории задачи) — таблица правил, а не парсер произвольной гармонии
- **Замена сложных обозначений — конвенция, а не расчёт по терциям.** У
  sus-аккорда терции нет вовсе (ни мажорной, ни минорной), у уменьшённого и
  увеличенного она есть, но звучит иначе. «Единственно верного» упрощения не
  существует, поэтому выбор один на всё приложение: sus4/sus2 снимаются
  целиком, `dim`/`dim7`/`m7b5` — к минору, `+` — к мажору
- **Суффикс с цифрой не теряет её**: `7(sus4)` упрощается до `7`, `+7` — до
  `7` — снимается только сама пометка задержания или увеличения, а септима,
  если она была, остаётся
- **Обозначения вне списка не трогаются**: `7`, `m`, `maj7`, `9` и подобные
  остаются как есть — фильтр целится в перечисленные типы, а не «всё сложное»
- **Нераспознанное возвращается как есть** — по той же причине, что в
  `transposeChord`: косая черта встречается в сборнике не только в обращениях
- **Тональность песни считается по корню аккорда**, поэтому `songKey` и
  `preferSharp` упрощение не задевает: `G/B` и `G` дают одну и ту же тонику
- **Применяется сразу после сдвига**, до разбора повторов и разметки в HTML:
  обе операции работают по `{…}`, и делать их после превращения в теги было бы
  разбором собственной разметки
- **Настройка входит в `watch` пересчёта раскладки**: «D7/F#» вдвое шире «D7»,
  «Bb7(sus4)» шире «Bb7», а от ширины надписей зависит, какие из них расходятся
- E2E — `chords.spec.js` (тумблер «упростить для гитары»: обращения из
  пересобранной фикстуры на 19 песнях + четвёртый куплет `CHORDS_BASS` с
  sus4/dim/m7b5/+) и `settings.spec.js` (гейт страницы, блокировка тумблеров,
...
бемолями, ре-мажор — диезами), но на гитаре диезы читать привычнее независимо от
тональности. Тумблер `forceSharp` (геттер `sharpForced`) эту конвенцию не
спрашивает вовсе.

- **Работает и без сдвига тональности.** `transposeText` раньше выходила рано
  при нулевом сдвиге (`!semitones`) — текст песни оставался как в файле, бемоли
  включительно. Условие раннего выхода теперь `!semitones && !sharp`: при
  включённом тумблере функция проходит по тексту и с нулевым сдвигом,
  переписывая ноты по `noteName(pitch, true)`
- **Перебивает `preferSharp`, а не дополняет его.** `sharpSpelling` в
  `SongDisplay.vue` при `settings.sharpForced` возвращает `true` без обращения
  к тексту песни — вычислять целевую тональность незачем, если её всё равно
  проигнорируют
